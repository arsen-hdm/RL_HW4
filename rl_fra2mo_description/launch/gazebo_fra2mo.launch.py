...
def generate_launch_description():
    ...
    rviz_config_file = os.path.join(get_package_share_directory('rl_fra2mo_description'), 'conf', 'explore.rviz')
    ...
    robot_localization_node = Node(
       ...
       output='screen',
       parameters=[os.path.join(get_package_share_directory('rl_fra2mo_description'), "config/ekf.yaml"), {"use_sim_time": True}],
    )
    ...
    nodes_to_start = [robot_state_publisher_node, joint_state_publisher_node, *ign, bridge, 
                      odom_tf, laser_id_link_tf, ign_clock_bridge, robot_localization_node]
